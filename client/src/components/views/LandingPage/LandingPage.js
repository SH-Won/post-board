...
import {getFirstProduct,getProduct,deleteProduct,getFilterProduct,getSearchProduct} from '../../../_actions/product_actions';
...
function LandingPage() {
    ...
    const products = useSelector(state=>state.upload.products,[state=>state.upload.products])
    ...
    const deleteProducts =(product_id,writer,product) =>{
      ...
      //  let newProducts = [...products]
        ...
        alert('정말로 삭제하시겠습니까?')
        dispatch(deleteProduct(product_id,writer,Skip,Limit,filters,searchTerm))
        .then(response=>{
            if(response.payload.success){
    ...
    products.length >0 && products.map((product,index)=>{
            ...
            const deleteButton = userId.userData.isAuth && user ===product.writer._id ?
            [<Button onClick={()=>deleteProducts(product._id,product.writer,product)}> 삭제 </Button>]:[<Button>좋아요</Button>]

        return <Col key={`${product._id}+${index}`} lg={6} md={8} xs={24}>
             <Card
               hoverable={true}
              ...
              <Meta 
                title={product.title}
                description={`${product.writer.name}  ${product.category}`}
                />
    ...
    return (
        <div style={{width:'90%' ,margin:'2rem auto'}}>
            <CheckBox list={category} handleFilter={filters=>handleFilter(filters,'category')}/>
            <br/><br/>
